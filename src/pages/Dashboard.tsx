...
export const Dashboard: React.FC = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const mCollections = monthlyCollectionsData?.reduce((acc, curr) => acc + Number(curr.amount || 0), 0) || 0;
      ...
      // 9. Top Selling & Profitable Products Analytics
      const { data: items } = await supabase
        .from('sale_items')
        .select('product_name, quantity, total_amount, total_profit')
        .is('deleted_at', null);

      const prodMap: Record<string, { name: string; qty: number; sales: number; profit: number }> = {};
      items?.forEach((it) => {
        if (!prodMap[it.product_name]) {
          prodMap[it.product_name] = { name: it.product_name, qty: 0, sales: 0, profit: 0 };
        }
        prodMap[it.product_name].qty += Number(it.quantity || 0);
        prodMap[it.product_name].sales += Number(it.total_amount || 0);
        prodMap[it.product_name].profit += Number(it.total_profit || 0);
      });

      const prodList = Object.values(prodMap);
      setTopSellingProducts([...prodList].sort((a, b) => b.qty - a.qty).slice(0, 5));
      setTopProfitableProducts([...prodList].sort((a, b) => b.profit - a.profit).slice(0, 5));
      ...
      const trendData = last7Days.map((dayStr) => {
        ...
        const profit = daySales.reduce((sum, s) => sum + Number(s.total_profit || 0), 0);
        const formattedDay = new Date(dayStr).toLocaleDateString('tr-TR', { weekday: 'short', day: 'numeric' });
        return { name: formattedDay, Satış: total, Kâr: profit };
      });
  ...
  const targetPercentage = stats.profitTarget > 0 ? Math.min(100, Math.round((stats.monthlyProfit / stats.profitTarget) * 100)) : 0;
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-4 sm:p-6 rounded-2xl">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight">Genel Bakış & Performans</h2>
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            Bu ayki toplam satış, kâr hedefi, müşteri alacakları ve stok durumunun anlık özeti.
          </p>
        </div>
        <button
      ...
      <div className="bg-slate-900/90 border border-slate-800/80 rounded-2xl p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider">Bugünün Özet Performansı</h3>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800/60">
            <span className="text-xs text-slate-400 font-medium block">Bugünkü Kâr</span>
            <span className="text-lg font-extrabold text-brand-400">{formatCurrency(stats.todayProfit)}</span>
            <span className="text-[11px] text-slate-500 block mt-0.5">Birim Maliyet Düşülmüş</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl relative overflow-hidden shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Bu Ay Tahsilat</span>
            <div className="p-2.5 bg-emerald-500/10 text-emerald-400 rounded-xl">
              <Receipt className="w-5 h-5" />
            </div>
          </div>
          <p className="text-2xl font-extrabold text-emerald-400 mt-3">{formatCurrency(stats.monthlyCollections)}</p>
          <p className="text-xs text-slate-400 mt-1">Nakit & Havale Toplamı</p>
        </div>

        {/* Monthly Profit */}
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl relative overflow-hidden shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Bu Ay Net Kâr</span>
            <div className="p-2.5 bg-indigo-500/10 text-indigo-400 rounded-xl">
              <DollarSign className="w-5 h-5" />
            </div>
          </div>
          <p className="text-2xl font-extrabold text-indigo-400 mt-3">{formatCurrency(stats.monthlyProfit)}</p>
          <p className="text-xs text-slate-400 mt-1">Gerçekleşen Kâr</p>
        </div>

        {/* Profit Target */}
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl relative overflow-hidden shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Hedef Gerçekleşme</span>
            <div className="p-2.5 bg-purple-500/10 text-purple-400 rounded-xl">
              <Target className="w-5 h-5" />
            </div>
          </div>
          <div className="flex items-baseline justify-between mt-3">
            <span className="text-2xl font-extrabold text-white">%{targetPercentage}</span>
            <span className="text-xs text-slate-400 font-medium">Hedef: {formatCurrency(stats.profitTarget)}</span>
          </div>
          ...
          <p className="text-[11px] text-slate-400 mt-2">
            Hedefe Kalan Kâr: <span className="font-bold text-slate-200">{formatCurrency(stats.remainingProfitTarget)}</span>
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900/70 border border-slate-800 p-4 rounded-xl">
          <span className="text-xs text-slate-400 font-medium block">Toplam Müşteri Alacağı</span>
          <span className="text-xl font-bold text-amber-400 block mt-1">{formatCurrency(stats.totalCustomerDebt)}</span>
          <span className="text-[11px] text-slate-500 block mt-0.5">Piyasadaki Toplam Alacak</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-lg flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-bold text-white text-base">Son 7 Günlük Satış ve Kâr Trendi</h3>
              <p className="text-xs text-slate-400">Günlük ciro ve gerçekleşen kâr grafik görünümü</p>
            </div>
          </div>
          <div className="h-64 w-full mt-2">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={salesTrend}>
                ...
                <Area type="monotone" dataKey="Kâr" stroke="#10b981" strokeWidth={2.5} fillOpacity={1} fill="url(#colorKar)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
              ...
              recentSales.map((s) => (
                <div key={s.id} className="flex items-center justify-between p-3 bg-slate-950/70 rounded-xl border border-slate-800/60">
                  ...
                  <div className="text-right">
                    <span className="text-sm font-extrabold text-white block">{formatCurrency(s.total_amount)}</span>
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-white text-base">Kritik Stok Uyarısı Veren Ürünler</h3>
            </div>
            <Link to="/products" className="text-xs text-brand-400 hover:underline">Stok Kartları</Link>
          </div>
          <div className="space-y-2.5">
            {criticalProducts.length === 0 ? (
              <div className="p-6 bg-slate-950/40 rounded-xl text-center border border-slate-800/40">
                <p className="text-xs text-emerald-400 font-semibold">Tüm ürünlerin stok seviyesi yeterli düzeyde!</p>
              </div>
            ) : (
              criticalProducts.map((p) => (
                <div key={p.id} className="flex items-center justify-between p-3 bg-amber-950/20 border border-amber-800/30 rounded-xl">
                  <div>
                    <span className="text-xs font-bold text-slate-200 block">{p.product_name}</span>
                  ...
                  <div className="text-right">
                    <span className="text-xs font-extrabold text-amber-400 block">
                      {formatNumber(p.current_stock)} {p.unit} Kaldı
                    </span>
                    <button
                      onClick={() => openStockEntryModal(p.id)}
                      className="text-[11px] text-brand-400 hover:underline mt-0.5 font-medium block"
                    >
                      + Mal Girişi Yap
                    </button>
                  </div>
