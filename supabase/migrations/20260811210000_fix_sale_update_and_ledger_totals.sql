...
-- Migration: Fix Order Edit Ledger Sync & Remove Fake Credit Payments on Order Reductions
...
-- 2. UPDATED UPDATE_SALE_TRANSACTION PROCEDURE
...
BEGIN
  ...
  -- Clean up old DÜZELTME / İADE entries for this sale
  DELETE FROM public.customer_ledger
  WHERE sale_id = p_sale_id AND owner_id = v_owner_id AND movement_type IN ('DÜZELTME', 'İADE');

  -- Update original BORÇ row in customer_ledger directly to new_total_amount
  ...
  IF v_ledger_exists > 0 THEN
    UPDATE public.customer_ledger
    SET debit = v_new_total_amount,
        description = 'Vadeli Satış #' || v_sale_number || CASE WHEN v_total_diff <> 0 THEN ' (Güncellendi)' ELSE '' END
    WHERE sale_id = p_sale_id AND owner_id = v_owner_id AND movement_type = 'BORÇ';
  ELSE
    INSERT INTO public.customer_ledger (
      ...
      'Vadeli Satış #' || v_sale_number, v_new_total_amount, 0.00, 0.00
    );
  END IF;

  -- Recalculate running balances across all customer ledger entries
  PERFORM public.recalculate_all_customer_ledger_balances();
...
-- 3. UPDATED CANCEL_SALE_TRANSACTION PROCEDURE
...
BEGIN
  ...
  -- Update customer_ledger BORÇ row to 0 so it doesn't inflate total purchases
  UPDATE public.customer_ledger
  SET debit = 0.00,
      description = 'Sipariş İptal Edildi (#' || v_sale_number || ')'
  WHERE sale_id = p_sale_id AND owner_id = v_owner_id AND movement_type = 'BORÇ';

  -- Clean up old DÜZELTME / İADE ledger entries for this sale
  DELETE FROM public.customer_ledger
  WHERE sale_id = p_sale_id AND owner_id = v_owner_id AND movement_type IN ('DÜZELTME', 'İADE');
