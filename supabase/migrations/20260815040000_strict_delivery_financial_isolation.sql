...
-- MIGRATION: 20260815040000_strict_delivery_financial_isolation.sql
-- TÜM SATIŞ TÜRLERİNDE (NORMAL, ÖN SİPARİŞ, HIZLI SATIŞ) CARİ BORÇ, ÖDEME PLANI VE PDF/WHATSAPP'I SADECE "TESLİM EDİLDİ" AŞAMASINA BAĞLAMA

-- 1. SALES TABLOSUNA GEREKLİ KOLONLARIN GEÇERLİ KILINMASI
...
-- ----------------------------------------------------------------------------
-- 2. CREATE_SALE_TRANSACTION RPC GÜNCELLEMESİ:
--    SATIŞ OLUŞTURULDUĞUNDA STOK DÜŞER, ANCAK TESLİM EDİLENE KADAR CARİ BORÇ VE ÖDEME PLANI YAZMAZ!
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Sales tablosuna kaydı ekle (order_status = 'received', delivered_at = NULL)
  ...
  -- KESİNLEŞTİRME KURALLARI:
  -- Teslimat yapılmadığı sürece (delivered_at IS NULL) customer_ledger borç kaydı ve payment_schedules EKLENMEZ.
...
-- ----------------------------------------------------------------------------
-- 3. CONFIRM_DELIVERY_AND_FINALIZE_SALE_TRANSACTION RPC GÜNCELLEMESİ:
--    SADECE TESLİM EDİLDİĞİ ANDA CARİYE BORÇ EKLER VE ÖDEME PLANINI TESLİM TARİHİNDEN BAŞLATIR!
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Teslimat zamanı ve vadesini belirle (Varsayılan: Europe/Istanbul şu anki zaman)
  ...
  -- 2. MÜŞTERİ CARİSİNE BORÇ EKLEME (SADECE TESLİM ANINDA GİRER - Madde 2 & 10)
  ...
  IF v_ledger_exists = 0 THEN
    ...
    -- Peşin satışsa peşin ödeme kaydı ekle
  ...
  -- 3. ÖDEME PLANINI TESLİM TARİHİNDEN BAŞLATARAK OLUŞTUR (Madde 3, 11 & 12)
