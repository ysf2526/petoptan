...
-- Migration: Order Editing and Order Cancellation Atomic Stored Procedures

-- 1. UPDATE SALE TRANSACTION RPC
...
BEGIN
  ...
  -- Adjust customer ledger by net total diff
  IF v_total_diff <> 0 THEN
    IF v_total_diff > 0 THEN
      INSERT INTO public.customer_ledger (
        ...
        'Sipariş Güncelleme Borç Artışı (#' || v_sale_number || ')', v_total_diff, 0.00, 0.00
      );
    ELSE
      INSERT INTO public.customer_ledger (
        owner_id, customer_id, sale_id, movement_type, description, debit, credit, balance
        ...
        'Sipariş Güncelleme Borç İndirimi (#' || v_sale_number || ')', 0.00, ABS(v_total_diff), 0.00
      );
    END IF;
  END IF;
...
BEGIN
  ...
  -- Zero out customer debt in ledger if remaining debt > 0
  IF v_remaining_debt > 0 THEN
    INSERT INTO public.customer_ledger (
      owner_id, customer_id, sale_id, movement_type, description, debit, credit, balance
    ) VALUES (
      v_owner_id, v_customer_id, p_sale_id, 'İADE',
      'Sipariş İptali İadesi (#' || v_sale_number || ')', 0.00, v_remaining_debt, 0.00
    );
  END IF;
